...
;;
;; This was broken on both, differently: JS emitted the inlined child as `extends SeqFault` (the
...
;; :extends chain.
;;
;;   1. PRECISE: catch the thrown Oob as Oob, reading its structured field.
;;   2. BASE: catch it as SeqFault (one level up).
;;   3. ROOT: catch it as Error (two levels up, the ambient host class).
;;   4. SIBLING isolation: a NoKey is NOT an Oob; the SeqFault arm takes it.
(
    (import "21_imported_error_tower_lib.lisp")

    (try (at [10 20] 5)
         catch e :of Oob (console.log "1 precise" e.message e.index))

    (try (at [10 20] 7)
         catch e :of SeqFault (console.log "2 base" e.message))

    (try (at [10 20] 9)
         catch e :of Error (console.log "3 root" e.message))

    (try (throw (new NoKey "no such key" "user_id"))
         catch e :of Oob (console.log "4 WRONG-sibling-matched")
         catch e :of SeqFault (console.log "4 sibling-ok" e.message))
)
