...
;;
;; A `catch :of ValueError` where ValueError is defined in another module used to be JS-broken: the
;; catch filter emitted `instanceof ValueError` -- the SOURCE name -- while the import inliner had
;; renamed the class to `__ll_inlined_ValueError_1`, so it was a ReferenceError. The boolean
;; `(x :of ValueError)` guard was fine, because it lowers to `__ll_is_type(v, "ValueError")` -- a test
...
(
    ...
    (try (parse-or-throw "x")
         catch e :of ValueError (console.log "1 caught-typed" e.message))
