...
examples/       374 .lisp programs. This is the end-to-end suite, not a demo folder.
...
Baseline to hold, measured at the conditions/lifted-frames sweep (2026-07-31): **C 319 passing /
0 failing / 2 refused / 0 not-yet**, over a 374-file total (13 library, 1 fixture, 31 negative). Any movement is
...
new refusal *among the 374* is a regression, not a backlog item, and should be read that way.
...
| a nested `fn` in a generator | **fixed** — `inGenerator` no longer leaks in | `80-adversarial/nested_fn_inside_generator.lisp` |
...
gate because no corpus file happened to write an indexed `:step`. Three of the six emitted C that does
not compile — a defect the corpus cannot even express as a failing test, since a file that will not
build has no output to compare against a golden.
