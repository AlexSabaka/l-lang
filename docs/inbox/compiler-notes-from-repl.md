...
| **3** | no raw message — string-surgery on formatted output | ✅ **FIXED.** `RuleValidationMessage` now carries `text` (the raw sentence), `column` and `location`; `message` is unchanged, so every existing caller is untouched. The REPL's ANSI-stripping regex is **deleted**, not kept as a fallback — a fallback that never runs is how a seam quietly stops being load-bearing. Also fixed the sharp edge the item flagged in passing: `formatMessage` **threw** out-of-Context (`getSource` → *File not found in the AST cache*), so merely *reading* `.message` could take the process down; it now degrades to the sentence. ~~Gated end-to-end-anchored in `test:repl`.~~ That anchor went with the REPL (D105); the fix itself stands and is exercised by every `RuleValidationMessage` consumer, but it is no longer pinned end-to-end. |
...
| **6** | dead README links | ⚠️ **MOOT for its own half.** `docs/repl.md` was deleted by D105 along with the REPL, and every link to it removed in the same commit. The other five stale `docs/development/**` links: still open. |
