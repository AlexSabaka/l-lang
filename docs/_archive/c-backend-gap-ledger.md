> **ARCHIVED 2026-07-28 — the probe record, superseded in its framing by D86.**
>
> This document measured the C backend *as an adversarial probe*: how far a native backend could get
> against the HIR contract, with the JavaScript backend as the oracle deciding who was right. **D86
> reversed that.** C is the reference implementation and JS is a frozen second implementation whose
> value is that a disagreement is worth looking at. So §1–§8's method, dip counts and frontier
> describe a relationship the project no longer has, and its "silent-wrong" verdicts were reached
> under the opposite assumption about which backend defines correctness.
>
> **The dated findings log (§9–§21) is NOT here** — it is live, still cited by 22 references from
> `src/` and the corpus, and lives at [`docs/spec/C-BACKEND-FINDINGS.md`](../spec/C-BACKEND-FINDINGS.md)
> with its section numbers preserved.
>
> Kept because the A1–A9 dip taxonomy it measures is still the vocabulary
> [`hir-llvm-consumption-spec.md`](../inbox/hir-llvm-consumption-spec.md) and the live `GapLedger.ts`
> use, and because the measurement — 3287 dips across 60 files — is the baseline any future
> readiness claim is compared against.
