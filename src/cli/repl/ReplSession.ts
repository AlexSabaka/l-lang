...
export class ReplSession {
  ...
  /** name -> the type it was first bound at. Enforces D23's one-name-one-type. See retypes(). */
  ...
  /**
   * Refuse a rebinding that would give an existing name a DIFFERENT type. D23: one name, one
   ...
   * BLOCK (D23). Loaded as a single cell, every binding in it would be block-scoped and invisible to
  ...
  private emitTail(
    ...
    // `compile()` does this on its first line, and we do not call `compile()`. Without it `rootSource`
    // is undefined, so `isImportedSymbol` bails on every symbol, NOTHING is ever inlined, and
    // `(import "std/math")` type-checks clean and then dies at run time with `sqr is not defined`.
    // An import in the REPL simply did not work. `beginProgram` is the seam compile() now shares.
    tx.beginProgram(root);
    ...
    // `visitProgram`/`compile` -- which we deliberately do not call -- are what normally drain these.
    // They are a public seam now (JSTransformerAstVisitor's "external-driver" block), so the three
    // `as any` casts docs/inbox/compiler-notes-from-repl.md #9 asked about are gone.
    const inlined = tx.getInlinedDefinitions();
    const operators = tx.getOperatorRegistrations();
    tx.populateTypesMetadata();
