...
const CASES: Case[] = [
  ...
  },
  {
    ...
    wasBroken:
      "D23, and NOT what the ruling first assumed. The checker treats a second `(let x ...)` as an " +
  ...
  },
  {
    ...
    cells: 3,
    wasBroken:
      "a file is written wrapped in ONE outer list, and that outer list is a BLOCK (D23). Loaded as a " +
  ...
  // -----------------------------------------------------------------------------------------------
  // The dev-sync seam. Found by DRIVING the REPL after the merge -- not by any of the 23 cases above.
  // -----------------------------------------------------------------------------------------------
  {
    name: "an IMPORT works -- its definitions are inlined",
    steps: [
      { input: '(import "std/math")' },
      { input: "(sqr 4)" },
      { input: "(pow 2 3)" },
    ],
    // An `(import ...)` yields no value -- it is accepted, silently, and enters history. What must be
    // asserted is that the two CALLS after it resolve, which is exactly what was broken.
    expect: [{ output: [] }, { value: "16" }, { value: "8" }],
    cells: 3,
    wasBroken:
      "AN IMPORT IN THE REPL DID NOT WORK, and the type checker was happy about it. `rootSource` is " +
      "set on the FIRST LINE of JSTransformerAstVisitor.compile() -- and the REPL deliberately does " +
      "not call compile(); it drives the visitor itself so it can visit history while emitting only " +
      "the new cell. So `rootSource` was undefined, `isImportedSymbol` (`source !== this.rootSource`) " +
      "bailed on EVERY symbol, nothing was ever inlined, and `(import \"std/math\")` type-checked " +
      "CLEAN and then died at run time with `sqr is not defined`. " +
      "The fix is a seam, not a patch: `beginProgram()` is public, compile() calls it too, and the " +
      "two set-up paths cannot drift again. It also retires three `as any` casts into private state " +
